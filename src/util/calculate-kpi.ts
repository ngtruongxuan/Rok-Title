...
export const calculateKpi = (
  ...
  return {
    ...
    power: Number(governorKPI.governor.power).toLocaleString("en-US"),
    ...
    kpi: Number(kpi).toLocaleString("en-US"),
    kpiNeeded: Number(kpiNeeded).toLocaleString("en-US"),
    kpiRemaining: Number(Math.round(kpiRemaining * 10) / 10).toLocaleString("en-US"),
